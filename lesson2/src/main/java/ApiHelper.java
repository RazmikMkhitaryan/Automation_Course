...
public class ApiHelper {
    ...
    public static JsonObject uploadPhoto(String userKey) throws IOException, InterruptedException {
        ...
        Set<org.openqa.selenium.Cookie> cookies = getDriver().manage().getCookies();
        Iterator<org.openqa.selenium.Cookie> iterator = cookies.iterator();
        while (iterator.hasNext()) {
            if (iterator.next().getName() == "afUserId") {

                org.openqa.selenium.Cookie cookie2 = new Cookie("afUserId", iterator.next().getValue());
